...
**Files:**
- Create: `python_worker/workflow/agent_registry.py`
- Create: `python_worker/tests/test_agent_registry.py`
...
from workflow.agent_registry import AGENT_ROLES, execute_agent
...
def test_execute_agent_unknown_role():
    from models.ppt_state import PPTState, Slide
    state = PPTState(slides=[Slide(slide_number=1, elements=[])], source_file="/tmp/test.pptx")
...
Expected: ImportError for `workflow.agent_registry`.
...
def _execute_theme_agent(ppt_state: PPTState, config: AgentNodeConfig, role: AgentRole) -> PPTState:
    ...
    messages = build_ppt_editing_messages(
        prompt=config.prompt or "Apply style changes",
...
```bash
git add python_worker/workflow/agent_registry.py python_worker/tests/test_agent_registry.py
git commit -m "feat: add agent registry with predefined roles and tool bindings
